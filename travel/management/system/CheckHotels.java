...
public class CheckHotels extends JFrame implements Runnable{
    ...
    public void run(){
        String[] text=new String[]{"Hotel Ideal","Hotel Mewad Inn","Sayaji Hotel","Hotel Vivan","Hotel Villa Empire","Rester Select Hotel","Hotel Prince Inn","Hotel Royal","Hotel Millennium","Hotel Landmark"};
        try{
            for(int i=0;i<=9;i++){
                label[i].setVisible(true);
                caption.setText(text[i]);
                Thread.sleep(2500);
                if(i<9) label[i].setVisible(false);
            }
        }
        catch(Exception e){
            e.printStackTrace();
